<?php
    ...
    $sql_ventas_pagadas = mysqli_query($conexion, "SELECT * FROM ventas WHERE estado_v=0 and id_usuario_venta='$id'");
            ...
            <tbody>
                <?php while($datos = mysqli_fetch_array($sql_ventas_pagadas)){ ?>
                    <tr>
                        ...
                        <td>
                            <p style="color:red;font-size:18px;font-family:monospace;"><?php echo $datos['total_venta']; ?></p>
                        </td>
                        <td>
                            <a href="imprimir_venta.php?id=<?php echo $datos['id_venta']; ?>"
                               class="btn btn-danger" target="_blank">Imprimir</a> 
                        </td>
                    </tr>
                <?php } ?>
            </tbody>
